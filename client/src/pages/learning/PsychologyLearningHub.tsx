...
// 3D psychology/mind background (purple + pink theme)
function PsychologyBG() {
  return (
    <div className="absolute inset-0 -z-10">
      <Suspense fallback={<div className="absolute inset-0 bg-[#1a0b1a]" />}>
        <Canvas camera={{ position: [0, 0, 6] }} dpr={[1, 2]}>
          <color attach="background" args={["#1a0b1a"]} />
          <ambientLight intensity={0.3} />
          <pointLight position={[5, 5, 5]} color="#a855f7" intensity={1.1} />
          <pointLight position={[-5, -3, 3]} color="#ec4899" intensity={0.9} />
          <Float speed={2} rotationIntensity={2} floatIntensity={1.5}>
            <mesh>
              <octahedronGeometry args={[0.7, 0]} />
              <meshStandardMaterial color="#a855f7" emissive="#9333ea" emissiveIntensity={0.6} transparent opacity={0.35} />
            </mesh>
          </Float>
          ...
          <Float speed={1.6} rotationIntensity={1.5}>
            <mesh position={[-2.4, 1.2, -0.6]}>
              ...
              <meshStandardMaterial color="#c084fc" emissive="#a855f7" emissiveIntensity={0.55} transparent opacity={0.28} />
            </mesh>
          </Float>
      ...
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(60%_40%_at_50%_50%,rgba(168,85,247,.3),transparent),radial-gradient(50%_50%_at_80%_80%,rgba(236,72,153,.25),transparent),radial-gradient(40%_40%_at_20%_20%,rgba(192,132,252,.2),transparent)]" />
    </div>
  );
}

// --- Micro Simulations (Psych) ---
function MemoryCurveSim() {
  const [days, setDays] = useState(7);
  // Ebbinghaus forgetting curve: retention = 100 * e^(-days/decay)
  const decay = 3;
  const retention = 100 * Math.exp(-days / decay);
  const points: [number, number][] = [];
  for (let d = 0; d <= 30; d++) {
    const r = 100 * Math.exp(-d / decay);
    points.push([(d / 30) * 100, 50 - (r / 100) * 40]);
  }
  return (
    <div className="rounded-2xl bg-white/5 ring-1 ring-white/10 p-4">
      <div className="mb-2 flex items-center gap-2 text-sm text-white/80">
        <Brain className="h-4 w-4 text-purple-300" /> Memory Retention
      </div>
      <div className="text-xs text-white/60 mb-3">After {days} days: {retention.toFixed(1)}% retained</div>
      <svg viewBox="0 0 100 50" className="w-full h-32">
        <path d={`M ${points.map(p => `${p[0]},${p[1]}`).join(" L ")}`} fill="none" stroke="#a855f7" strokeWidth="2" />
        <circle cx={(days / 30) * 100} cy={50 - (retention / 100) * 40} r="2.5" fill="#ec4899" />
        ...
        <text x="50" y="45" fontSize="7" fill="rgba(255,255,255,.6)" textAnchor="middle">Days</text>
        <text x="5" y="10" fontSize="7" fill="rgba(255,255,255,.6)">100%</text>
        <text x="5" y="48" fontSize="7" fill="rgba(255,255,255,.6)">0%</text>
      </svg>
      <label className="text-xs text-white/70 block mt-2">Days since learning
        <input type="range" min={0} max={30} value={days} onChange={(e) => setDays(+e.target.value)} className="w-full" />
      </label>
    </div>
  );
}

function CognitiveLoadSim() {
  const [tasks, setTasks] = useState(3);
  const maxCapacity = 7; // Miller's magic number ±2
  const load = (tasks / maxCapacity) * 100;
  const efficiency = load > 100 ? 0 : 100 - (load * 0.8);
  return (
    <div className="rounded-2xl bg-white/5 ring-1 ring-white/10 p-4">
      <div className="mb-2 flex items-center gap-2 text-sm text-white/80">
        <Heart className="h-4 w-4 text-pink-300" /> Cognitive Load
      </div>
      <div className="text-xs text-white/60 mb-3">{tasks} tasks → {load.toFixed(0)}% capacity ({efficiency.toFixed(0)}% efficiency)</div>
      <svg viewBox="0 0 100 50" className="w-full h-32">
        {/* Capacity bar */}
        <rect x="10" y="20" width="80" height="15" fill="rgba(255,255,255,.1)" rx="2" />
        <rect x="10" y="20" width={(load / 100) * 80} height="15" fill={load > 100 ? "#ef4444" : load > 80 ? "#f59e0b" : "#a855f7"} rx="2" />
        <text x="50" y="30" fontSize="8" fill="rgba(255,255,255,.8)" textAnchor="middle" fontWeight="bold">
          {load > 100 ? "Overloaded!" : `${load.toFixed(0)}%`}
        </text>
        <line x1="0" x2="100" y1="45" y2="45" stroke="rgba(255,255,255,.2)" strokeWidth="1" />
        <text x="50" y="52" fontSize="7" fill="rgba(255,255,255,.6)" textAnchor="middle">Working Memory Capacity</text>
      </svg>
      <label className="text-xs text-white/70 block mt-2">Concurrent tasks
        <input type="range" min={1} max={10} value={tasks} onChange={(e) => setTasks(+e.target.value)} className="w-full" />
      </label>
      <div className="mt-2 text-xs text-white/60">Optimal: 3-5 tasks (Miller's 7±2 rule)</div>
    </div>
  );
}
...
function ResourceCard({ r, bookmarked, onBookmark }: { r: Resource; bookmarked?: boolean; onBookmark?: () => void }) {
  return (
    <motion.a href={r.link} target="_blank" rel="noreferrer" whileHover={{ y: -4 }}
      ...
      <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-purple-500/10 to-pink-500/10 opacity-0 transition-opacity group-hover:opacity-100" />
...
// --- Main Psych Hub ---
export default function PsychologyLearningHub() {
  const { discipline = "psychology" } = useParams();
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        console.error("Error loading psychology hub:", error);
        setLoading(false);
      }
    }
  ...
  useEffect(() => {
    ...
    const q = activeStage.quiz || [
      { q: `In "${activeStage.title}", what matters most?`, opts: ["Understanding human behavior & research methods", "Memorization", "Fashion", "Speed typing"], correct: "Understanding human behavior & research methods" },
      { q: "A key psych skill is…", opts: ["Critical thinking & empathy", "Mime", "Crosswords", "Origami"], correct: "Critical thinking & empathy" },
    ];
      ...
      <div className="relative min-h-screen">
        ...
        <div className="grid place-items-center min-h-screen">
          <div className="flex items-center gap-3 text-white/80">
            <Loader2 className="h-5 w-5 animate-spin" /> Loading your Psychology journey…
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="relative min-h-screen text-white">
      ...
      <header className="relative mx-auto max-w-6xl px-6 pt-14 text-center">
        <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={springy}>
          <div className="flex items-center justify-center gap-3 rounded-full bg-white/5 px-6 py-3 ring-1 ring-white/10 backdrop-blur mx-auto w-fit mb-6">
            ...
            <p className="font-semibold tracking-wide">Psychology Learning Hub</p>
          </div>
          <h1 className="text-balance bg-gradient-to-br from-white to-purple-200 bg-clip-text text-4xl sm:text-6xl font-extrabold text-transparent">
            Explore the mind, understand behavior, transform lives
          </h1>
          <p className="max-w-2xl mx-auto mt-3 text-white/80">
            A comprehensive psychology journey for future researchers, clinicians, and counselors. Discover the science of the mind — right here.
          </p>
          ...
          <div className="mt-6 relative grid place-items-center mx-auto w-fit">
            <svg width="120" height="120" viewBox="0 0 120 120" className="drop-shadow">
              ...
              <motion.circle
                ...
                stroke="url(#psychgrad)" strokeLinecap="round"
              ...
              />
              <defs>
                <linearGradient id="psychgrad" x1="0" x2="1" y1="0" y2="0">
              ...
              <div className="grid gap-6 p-6 sm:grid-cols-5">
                ...
                <div className="sm:col-span-3 space-y-6">
                  ...
                  <div className="rounded-2xl bg-black/30 p-4 ring-1 ring-white/10 backdrop-blur-md">
                    ...
                    <textarea
                      ...
                      placeholder="What psychological concepts, research methods, or insights did you explore?"
                ...
                <div className="sm:col-span-2 flex flex-col gap-4">
                  <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                    <div className="mb-2 flex items-center gap-2 text-sm text-white/80"><Sparkles className="h-4 w-4 text-purple-300" /> Mind Playground</div>
                  ...
                  <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                    ...
                    <div className="space-y-3">
                      {(activeStage.quiz || [
                        { q: `In "${activeStage.title}" what matters most?`, opts: ["Understanding human behavior & research methods", "Memorization", "Graphic design", "Speed typing"], correct: "Understanding human behavior & research methods" },
                        { q: "A key psych skill is…", opts: ["Critical thinking & empathy", "Mime", "Crosswords", "Origami"], correct: "Critical thinking & empathy" },
                      ]).map((Q: any, i: number) => (
                        <div key={i}>
                  ...
                  {(activeStage.regions?.Global?.projectIdeas?.length > 0) && (
                    <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                      <div className="mb-2 flex items-center gap-2 text-sm text-white/80"><Brain className="h-4 w-4 text-pink-300" /> Project ideas</div>
                      <div className="flex snap-x snap-mandatory overflow-x-auto gap-3 pb-2">
                        {activeStage.regions.Global.projectIdeas.map((idea: string, i: number) => (
                          <motion.div key={i} whileHover={{ y: -2 }} className="snap-start min-w-[280px] rounded-2xl bg-white/5 p-4 ring-1 ring-white/10 backdrop-blur">
                            <div className="mb-2 flex items-center gap-2 text-xs text-white/70"><Sparkles className="h-4 w-4 text-purple-200" /> Idea</div>
      ...
      <footer className="py-10 text-center text-xs text-white/50">Made for future psychologists 🧠</footer>
    </div>
  );
}
