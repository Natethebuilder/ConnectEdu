...
// 3D environmental/nature background (green + emerald theme)
function EnvironmentalBG() {
  return (
    <div className="absolute inset-0 -z-10">
      <Suspense fallback={<div className="absolute inset-0 bg-[#0a1a0f]" />}>
        <Canvas camera={{ position: [0, 0, 6] }} dpr={[1, 2]}>
          <color attach="background" args={["#0a1a0f"]} />
          <ambientLight intensity={0.3} />
          <pointLight position={[5, 5, 5]} color="#10b981" intensity={1.1} />
          <pointLight position={[-5, -3, 3]} color="#059669" intensity={0.9} />
          <Float speed={2} rotationIntensity={2} floatIntensity={1.5}>
            <mesh>
              <dodecahedronGeometry args={[0.7, 0]} />
              <meshStandardMaterial color="#10b981" emissive="#059669" emissiveIntensity={0.6} transparent opacity={0.35} />
            </mesh>
          </Float>
          <Float speed={2.4} rotationIntensity={2}>
            <mesh position={[2.2, -1.3, 0]}>
              <dodecahedronGeometry args={[0.6, 0]} />
              <meshStandardMaterial color="#059669" emissive="#047857" emissiveIntensity={0.6} transparent opacity={0.3} />
            </mesh>
          </Float>
          <Float speed={1.6} rotationIntensity={1.5}>
            <mesh position={[-2.4, 1.2, -0.6]}>
              <dodecahedronGeometry args={[0.5, 0]} />
              <meshStandardMaterial color="#34d399" emissive="#10b981" emissiveIntensity={0.55} transparent opacity={0.28} />
            </mesh>
          </Float>
          <Float speed={1.8} rotationIntensity={1.8}>
            <mesh position={[0, 2, -1]}>
              <dodecahedronGeometry args={[0.4, 0]} />
              <meshStandardMaterial color="#6ee7b7" emissive="#059669" emissiveIntensity={0.5} transparent opacity={0.26} />
            </mesh>
          </Float>
          <OrbitControls enableZoom={false} enablePan={false} autoRotate autoRotateSpeed={0.4} />
        </Canvas>
      </Suspense>
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(60%_40%_at_50%_50%,rgba(16,185,129,.3),transparent),radial-gradient(50%_50%_at_80%_80%,rgba(5,150,105,.25),transparent),radial-gradient(40%_40%_at_20%_20%,rgba(52,211,153,.2),transparent)]" />
    </div>
  );
}

// --- Micro Simulations (Env Sci) ---
function CarbonFootprintSim() {
  const [emissions, setEmissions] = useState(50);
  // Carbon offset calculation: trees needed = emissions / 20 (rough estimate)
  const treesNeeded = Math.ceil(emissions / 20);
  const offset = Math.min(100, (treesNeeded * 20 / emissions) * 100);
  
  return (
    <div className="rounded-2xl bg-white/5 ring-1 ring-white/10 p-4">
      <div className="mb-2 flex items-center gap-2 text-sm text-white/80">
        <TreePine className="h-4 w-4 text-emerald-300" /> Carbon Footprint
      </div>
      <div className="text-xs text-white/60 mb-3">{emissions} tons CO₂/year → {treesNeeded} trees needed ({offset.toFixed(0)}% offset)</div>
      <svg viewBox="0 0 100 50" className="w-full h-32">
        {/* Emissions bar */}
        <rect x="10" y="8" width="80" height="8" fill="rgba(255,255,255,.1)" rx="2" />
        <rect x="10" y="8" width={(emissions / 100) * 80} height="8" fill="#ef4444" rx="2" />
        <text x="50" y="15" fontSize="7" fill="rgba(255,255,255,.8)" textAnchor="middle" fontWeight="bold">Emissions: {emissions} tons</text>
        {/* Offset bar */}
        <rect x="10" y="20" width="80" height="8" fill="rgba(255,255,255,.1)" rx="2" />
        <rect x="10" y="20" width={(offset / 100) * 80} height="8" fill="#10b981" rx="2" />
        <text x="50" y="27" fontSize="7" fill="rgba(255,255,255,.8)" textAnchor="middle">Offset: {offset.toFixed(0)}%</text>
        {/* Trees visualization */}
        <text x="50" y="40" fontSize="8" fill="rgba(255,255,255,.8)" textAnchor="middle" fontWeight="bold">
          🌳 {treesNeeded} trees
        </text>
      </svg>
      <label className="text-xs text-white/70 block mt-2">Annual CO₂ emissions (tons)
        <input type="range" min={0} max={100} value={emissions} onChange={(e) => setEmissions(+e.target.value)} className="w-full" />
      </label>
      <div className="mt-2 text-xs text-white/60">Each tree offsets ~20 kg CO₂/year</div>
    </div>
  );
}

function EcosystemHealthSim() {
  const [biodiversity, setBiodiversity] = useState(70);
  const [pollution, setPollution] = useState(30);
  
  // Ecosystem health = biodiversity - pollution (with some scaling)
  const health = Math.max(0, Math.min(100, biodiversity - (pollution * 0.7)));
  const status = health > 70 ? "Healthy" : health > 40 ? "Moderate" : "At Risk";
  const statusColor = health > 70 ? "#10b981" : health > 40 ? "#f59e0b" : "#ef4444";
  
  return (
    <div className="rounded-2xl bg-white/5 ring-1 ring-white/10 p-4">
      <div className="mb-2 flex items-center gap-2 text-sm text-white/80">
        <Wind className="h-4 w-4 text-green-300" /> Ecosystem Health
      </div>
      <div className="text-xs text-white/60 mb-3">Biodiversity: {biodiversity}%, Pollution: {pollution}% → Health: {health.toFixed(0)}% ({status})</div>
      <svg viewBox="0 0 100 50" className="w-full h-32">
        {/* Biodiversity bar */}
        <rect x="10" y="8" width="80" height="8" fill="rgba(255,255,255,.1)" rx="2" />
        <rect x="10" y="8" width={(biodiversity / 100) * 80} height="8" fill="#10b981" rx="2" />
        <text x="50" y="15" fontSize="7" fill="rgba(255,255,255,.8)" textAnchor="middle">Biodiversity: {biodiversity}%</text>
        {/* Pollution bar */}
        <rect x="10" y="20" width="80" height="8" fill="rgba(255,255,255,.1)" rx="2" />
        <rect x="10" y="20" width={(pollution / 100) * 80} height="8" fill="#ef4444" rx="2" />
        <text x="50" y="27" fontSize="7" fill="rgba(255,255,255,.8)" textAnchor="middle">Pollution: {pollution}%</text>
        {/* Health indicator */}
        <rect x="10" y="32" width="80" height="8" fill="rgba(255,255,255,.1)" rx="2" />
        <rect x="10" y="32" width={(health / 100) * 80} height="8" fill={statusColor} rx="2" />
        <text x="50" y="39" fontSize="7" fill="rgba(255,255,255,.8)" textAnchor="middle" fontWeight="bold">{status}: {health.toFixed(0)}%</text>
      </svg>
      <div className="mt-3 grid grid-cols-2 gap-2">
        <label className="text-xs text-white/70">Biodiversity (%)
          <input type="range" min={0} max={100} value={biodiversity} onChange={(e) => setBiodiversity(+e.target.value)} className="w-full" />
        </label>
        <label className="text-xs text-white/70">Pollution (%)
          <input type="range" min={0} max={100} value={pollution} onChange={(e) => setPollution(+e.target.value)} className="w-full" />
        </label>
      </div>
    </div>
  );
...
// --- Main Environmental Science Hub ---
export default function EnvironmentalScienceLearningHub() {
  const { discipline = "environmental-science" } = useParams();
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        console.error("Error loading environmental science hub:", error);
        setLoading(false);
      }
    }
  ...
  useEffect(() => {
    ...
    const q = activeStage.quiz || [
      { q: `In "${activeStage.title}", what matters most?`, opts: ["Environmental research & sustainability", "Memorization", "Fashion", "Speed typing"], correct: "Environmental research & sustainability" },
      { q: "A key env sci skill is…", opts: ["Ecosystem analysis & conservation", "Mime", "Crosswords", "Origami"], correct: "Ecosystem analysis & conservation" },
    ];
      ...
      <div className="relative min-h-screen">
        ...
        <div className="grid place-items-center min-h-screen">
          <div className="flex items-center gap-3 text-white/80">
            <Loader2 className="h-5 w-5 animate-spin" /> Loading your Environmental Science journey…
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="relative min-h-screen text-white">
      ...
      <header className="relative mx-auto max-w-6xl px-6 pt-14 text-center">
        <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={springy}>
          <div className="flex items-center justify-center gap-3 rounded-full bg-white/5 px-6 py-3 ring-1 ring-white/10 backdrop-blur mx-auto w-fit mb-6">
            ...
            <p className="font-semibold tracking-wide">Environmental Science Learning Hub</p>
          </div>
          <h1 className="text-balance bg-gradient-to-br from-white to-emerald-200 bg-clip-text text-4xl sm:text-6xl font-extrabold text-transparent">
            Protect ecosystems, understand climate, sustain our planet
          </h1>
          <p className="max-w-2xl mx-auto mt-3 text-white/80">
            A comprehensive environmental science journey for future conservationists, climate scientists, and sustainability leaders. Explore nature and policy — right here.
          </p>
          ...
          <div className="mt-6 relative grid place-items-center mx-auto w-fit">
            <svg width="120" height="120" viewBox="0 0 120 120" className="drop-shadow">
              ...
              <motion.circle
                ...
                stroke="url(#envgrad)" strokeLinecap="round"
              ...
              />
              <defs>
                <linearGradient id="envgrad" x1="0" x2="1" y1="0" y2="0">
                ...
                <motion.div whileHover={{ y: -2 }} className="rounded-2xl bg-white/5 p-5 backdrop-blur ring-1 ring-white/10">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => setActiveStage(stage)} className="inline-flex items-center gap-1 rounded-full bg-emerald-500/20 px-3 py-1 text-emerald-200 ring-1 ring-emerald-300/30 hover:bg-emerald-500/30">
            ...
            >
              ...
              <div className="grid gap-6 p-6 sm:grid-cols-5">
                ...
                <div className="sm:col-span-3 space-y-6">
                  ...
                  <div className="rounded-2xl bg-black/30 p-4 ring-1 ring-white/10 backdrop-blur-md">
                    <h4 className="mb-3 font-semibold text-emerald-200">Curated resources</h4>
                  ...
                  <div className="rounded-2xl bg-black/30 p-4 ring-1 ring-white/10 backdrop-blur-md">
                    <h4 className="mb-3 font-semibold text-emerald-200">Reflection</h4>
                    <textarea
                      ...
                      placeholder="What environmental concepts, conservation strategies, or sustainability insights did you explore?"
                ...
                <div className="sm:col-span-2 flex flex-col gap-4">
                  <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                    <div className="mb-2 flex items-center gap-2 text-sm text-white/80"><Sparkles className="h-4 w-4 text-emerald-300" /> Nature Playground</div>
                  ...
                  <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                    <div className="mb-2 flex items-center gap-2 text-sm text-white/80"><GraduationCap className="h-4 w-4 text-emerald-300" /> Quick quiz</div>
                    <div className="space-y-3">
                      {(activeStage.quiz || [
                        { q: `In "${activeStage.title}" what matters most?`, opts: ["Environmental research & sustainability", "Memorization", "Graphic design", "Speed typing"], correct: "Environmental research & sustainability" },
                        { q: "A key env sci skill is…", opts: ["Ecosystem analysis & conservation", "Mime", "Crosswords", "Origami"], correct: "Ecosystem analysis & conservation" },
                      ]).map((Q: any, i: number) => (
                        <div key={i}>
                            ...
                            {Q.opts.map((opt: string) => (
                              <button key={opt} onClick={() => setQuizAnswers((prev) => ({ ...prev, [i]: opt }))}
                                className={cx("rounded-full border border-white/15 px-3 py-1 text-sm",
                                  quizAnswers[i] === opt ? "bg-emerald-500/30" : "bg-white/5 hover:bg-white/10")}>{opt}</button>
                            ))}
                          </div>
                        </div>
                      ))}
                  ...
                  {(activeStage.regions?.Global?.projectIdeas?.length > 0) && (
                    <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                      <div className="mb-2 flex items-center gap-2 text-sm text-white/80"><TreePine className="h-4 w-4 text-green-300" /> Project ideas</div>
                      <div className="flex snap-x snap-mandatory overflow-x-auto gap-3 pb-2">
                        {activeStage.regions.Global.projectIdeas.map((idea: string, i: number) => (
                          <motion.div key={i} whileHover={{ y: -2 }} className="snap-start min-w-[280px] rounded-2xl bg-white/5 p-4 ring-1 ring-white/10 backdrop-blur">
                            <div className="mb-2 flex items-center gap-2 text-xs text-white/70"><Sparkles className="h-4 w-4 text-emerald-200" /> Idea</div>
      ...
      <footer className="py-10 text-center text-xs text-white/50">Made for future environmental scientists 🌱</footer>
    </div>
  );
}
