...
// 3D mathematical/geometric background (indigo + purple mathematics theme)
function MathBG() {
  return (
    <div className="absolute inset-0 -z-10">
      <Suspense fallback={<div className="absolute inset-0 bg-[#0f0a1a]" />}>
        <Canvas camera={{ position: [0, 0, 6] }} dpr={[1, 2]}>
          ...
          <pointLight position={[5, 5, 5]} color="#6366f1" intensity={1.1} />
          <pointLight position={[-5, -3, 3]} color="#8b5cf6" intensity={0.9} />
          <Float speed={2} rotationIntensity={2} floatIntensity={1.5}>
            <mesh>
              <icosahedronGeometry args={[1.2, 0]} />
              <meshStandardMaterial color="#6366f1" emissive="#4f46e5" emissiveIntensity={0.6} transparent opacity={0.35} />
            </mesh>
          </Float>
          <Float speed={2.4} rotationIntensity={2}>
            <mesh position={[2.2, -1.3, 0]}>
              <icosahedronGeometry args={[0.9, 0]} />
              <meshStandardMaterial color="#8b5cf6" emissive="#7c3aed" emissiveIntensity={0.6} transparent opacity={0.3} />
            </mesh>
          </Float>
          <Float speed={1.6} rotationIntensity={1.5}>
            <mesh position={[-2.4, 1.2, -0.6]}>
              <icosahedronGeometry args={[0.7, 0]} />
              <meshStandardMaterial color="#a78bfa" emissive="#6366f1" emissiveIntensity={0.55} transparent opacity={0.28} />
            </mesh>
          </Float>
          <Float speed={1.8} rotationIntensity={1.8}>
            <mesh position={[0, 2, -1]}>
              <icosahedronGeometry args={[0.5, 0]} />
              <meshStandardMaterial color="#c4b5fd" emissive="#8b5cf6" emissiveIntensity={0.5} transparent opacity={0.26} />
            </mesh>
          </Float>
          <OrbitControls enableZoom={false} enablePan={false} autoRotate autoRotateSpeed={0.4} />
        </Canvas>
      </Suspense>
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(60%_40%_at_50%_50%,rgba(99,102,241,.3),transparent),radial-gradient(50%_50%_at_80%_80%,rgba(139,92,246,.25),transparent),radial-gradient(40%_40%_at_20%_20%,rgba(167,139,250,.2),transparent)]" />
    </div>
  );
}

// --- Micro Simulations (Math) ---
function FunctionPlotterSim() {
  ...
  const points = useMemo(() => {
    ...
    for (let x = -50; x <= 50; x += 2) {
      const xNorm = x / 50;
      const y = a * xNorm * xNorm + b * xNorm;
      const yNorm = 25 - (y * 20);
      pts.push([x + 50, Math.max(0, Math.min(50, yNorm))]);
    }
    return pts;
  }, [a, b]);
  ...
  return (
    <div className="rounded-2xl bg-white/5 ring-1 ring-white/10 p-4">
      <div className="mb-2 flex items-center gap-2 text-sm text-white/80">
        <BarChart3 className="h-4 w-4 text-indigo-300" /> Function Plotter
      </div>
      <div className="text-xs text-white/60 mb-3">f(x) = {a}x² {b >= 0 ? "+" : ""} {b}x</div>
      <svg viewBox="0 0 100 50" className="w-full h-32">
        {d && <path d={d} fill="none" stroke="#6366f1" strokeWidth="2" />}
        ...
        <text x="50" y="45" fontSize="8" fill="rgba(255,255,255,.6)" textAnchor="middle">y = {a}x² {b >= 0 ? "+" : ""} {b}x</text>
      </svg>
      <div className="mt-3 grid grid-cols-2 gap-3">
        <label className="text-xs text-white/70">Coefficient a
          <input type="range" min={-2} max={2} step={0.1} value={a} onChange={(e) => setA(+e.target.value)} className="w-full" />
        </label>
        <label className="text-xs text-white/70">Coefficient b
          <input type="range" min={-2} max={2} step={0.1} value={b} onChange={(e) => setB(+e.target.value)} className="w-full" />
        </label>
      </div>
    </div>
  );
...
function DerivativeVizSim() {
  const [x, setX] = useState(0);
  const f = (x: number) => x * x;
  const df = (x: number) => 2 * x;
  const y = f(x);
  const slope = df(x);
  const tangentY = y + slope * (0 - x);
  return (
    <div className="rounded-2xl bg-white/5 ring-1 ring-white/10 p-4">
      <div className="mb-2 flex items-center gap-2 text-sm text-white/80">
        <TrendingUp className="h-4 w-4 text-purple-300" /> Derivative
      </div>
      <div className="text-xs text-white/60 mb-3">f(x) = x², f'({x.toFixed(1)}) = {slope.toFixed(1)}</div>
      <svg viewBox="0 0 100 50" className="w-full h-32">
        {/* Function curve */}
        <path d="M 0 50 Q 25 25 50 12.5 T 100 0" fill="none" stroke="#8b5cf6" strokeWidth="2" />
        {/* Tangent line */}
        <line 
          x1={Math.max(0, (x + 2.5) * 10 - 20)} 
          x2={Math.min(100, (x + 2.5) * 10 + 20)} 
          y1={50 - (tangentY + slope * (-2.5 - x)) * 2} 
          y2={50 - (tangentY + slope * (2.5 - x)) * 2} 
          stroke="#6366f1" 
          strokeWidth="2" 
          strokeDasharray="4,2"
        />
        {/* Point */}
        <circle cx={(x + 2.5) * 10} cy={50 - y * 2} r="3" fill="#a78bfa" />
        <line x1="0" x2="100" y1="50" y2="50" stroke="rgba(255,255,255,.2)" strokeWidth="1" />
        <line x1="50" y1="0" x2="50" y2="50" stroke="rgba(255,255,255,.2)" strokeWidth="1" />
      </svg>
      <label className="text-xs text-white/70 block mt-2">x value
        <input type="range" min={-2.5} max={2.5} step={0.1} value={x} onChange={(e) => setX(+e.target.value)} className="w-full" />
      </label>
    </div>
  );
}
...
function ResourceCard({ r, bookmarked, onBookmark }: { r: Resource; bookmarked?: boolean; onBookmark?: () => void }) {
  return (
    <motion.a href={r.link} target="_blank" rel="noreferrer" whileHover={{ y: -4 }}
      ...
      <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-indigo-500/10 to-purple-500/10 opacity-0 transition-opacity group-hover:opacity-100" />
...
// --- Main Math Hub ---
export default function MathematicsLearningHub() {
  const { discipline = "mathematics" } = useParams();
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        console.error("Error loading mathematics hub:", error);
        setLoading(false);
      }
    }
  ...
  useEffect(() => {
    ...
    const q = activeStage.quiz || [
      { q: `In "${activeStage.title}", what matters most?`, opts: ["Mathematical reasoning & problem-solving", "Memorization", "Fashion", "Speed typing"], correct: "Mathematical reasoning & problem-solving" },
      { q: "A key math skill is…", opts: ["Abstract thinking & proof techniques", "Mime", "Crosswords", "Origami"], correct: "Abstract thinking & proof techniques" },
    ];
      ...
      <div className="relative min-h-screen">
        ...
        <div className="grid place-items-center min-h-screen">
          <div className="flex items-center gap-3 text-white/80">
            <Loader2 className="h-5 w-5 animate-spin" /> Loading your Mathematics journey…
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="relative min-h-screen text-white">
      ...
      <header className="relative mx-auto max-w-6xl px-6 pt-14 text-center">
        <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={springy}>
          <div className="flex items-center justify-center gap-3 rounded-full bg-white/5 px-6 py-3 ring-1 ring-white/10 backdrop-blur mx-auto w-fit mb-6">
            <Calculator className="h-6 w-6 text-indigo-300" />
            <p className="font-semibold tracking-wide">Mathematics Learning Hub</p>
          </div>
          <h1 className="text-balance bg-gradient-to-br from-white to-indigo-200 bg-clip-text text-4xl sm:text-6xl font-extrabold text-transparent">
            Unlock the language of the universe
          </h1>
          <p className="max-w-2xl mx-auto mt-3 text-white/80">
            A comprehensive mathematical journey for aspiring mathematicians. Explore proofs, functions, and abstract thinking — right here.
          </p>
          ...
          <div className="mt-6 relative grid place-items-center mx-auto w-fit">
            <svg width="120" height="120" viewBox="0 0 120 120" className="drop-shadow">
              ...
              <motion.circle
                ...
                stroke="url(#mathgrad)" strokeLinecap="round"
              ...
              />
              <defs>
                <linearGradient id="mathgrad" x1="0" x2="1" y1="0" y2="0">
            ...
            >
              ...
              <div className="grid gap-6 p-6 sm:grid-cols-5">
                ...
                <div className="sm:col-span-3 space-y-6">
                  ...
                  <div className="rounded-2xl bg-black/30 p-4 ring-1 ring-white/10 backdrop-blur-md">
                    <h4 className="mb-3 font-semibold text-indigo-200">Curated resources</h4>
                  ...
                  <div className="rounded-2xl bg-black/30 p-4 ring-1 ring-white/10 backdrop-blur-md">
                    <h4 className="mb-3 font-semibold text-indigo-200">Reflection</h4>
                    <textarea
                      ...
                      placeholder="What theorems, proofs, or insights did you explore?"
                ...
                <div className="sm:col-span-2 flex flex-col gap-4">
                  <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                    <div className="mb-2 flex items-center gap-2 text-sm text-white/80"><Sparkles className="h-4 w-4 text-indigo-300" /> Math Playground</div>
                  ...
                  <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                    <div className="mb-2 flex items-center gap-2 text-sm text-white/80"><GraduationCap className="h-4 w-4 text-indigo-300" /> Quick quiz</div>
                    <div className="space-y-3">
                      {(activeStage.quiz || [
                        { q: `In "${activeStage.title}" what matters most?`, opts: ["Mathematical reasoning & problem-solving", "Memorization", "Graphic design", "Speed typing"], correct: "Mathematical reasoning & problem-solving" },
                        { q: "A key math skill is…", opts: ["Abstract thinking & proof techniques", "Mime", "Crosswords", "Origami"], correct: "Abstract thinking & proof techniques" },
                      ]).map((Q: any, i: number) => (
                        <div key={i}>
                            ...
                            {Q.opts.map((opt: string) => (
                              <button key={opt} onClick={() => setQuizAnswers((prev) => ({ ...prev, [i]: opt }))}
                                className={cx("rounded-full border border-white/15 px-3 py-1 text-sm",
                                  quizAnswers[i] === opt ? "bg-indigo-500/30" : "bg-white/5 hover:bg-white/10")}>{opt}</button>
                            ))}
                          </div>
                        </div>
                      ))}
                  ...
                  {(activeStage.regions?.Global?.projectIdeas?.length > 0) && (
                    <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                      <div className="mb-2 flex items-center gap-2 text-sm text-white/80"><Calculator className="h-4 w-4 text-purple-300" /> Project ideas</div>
                      <div className="flex snap-x snap-mandatory overflow-x-auto gap-3 pb-2">
                        {activeStage.regions.Global.projectIdeas.map((idea: string, i: number) => (
                          <motion.div key={i} whileHover={{ y: -2 }} className="snap-start min-w-[280px] rounded-2xl bg-white/5 p-4 ring-1 ring-white/10 backdrop-blur">
                            <div className="mb-2 flex items-center gap-2 text-xs text-white/70"><Sparkles className="h-4 w-4 text-indigo-200" /> Idea</div>
      ...
      <footer className="py-10 text-center text-xs text-white/50">Made for future mathematicians ∞</footer>
    </div>
  );
}
