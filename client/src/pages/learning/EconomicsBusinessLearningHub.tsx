...
// 3D business/finance background (blue + gold economics theme)
function EconomicsBG() {
  return (
    <div className="absolute inset-0 -z-10">
      <Suspense fallback={<div className="absolute inset-0 bg-[#0a0f1a]" />}>
        <Canvas camera={{ position: [0, 0, 6] }} dpr={[1, 2]}>
          ...
          <pointLight position={[5, 5, 5]} color="#3b82f6" intensity={1.1} />
          <pointLight position={[-5, -3, 3]} color="#f59e0b" intensity={0.9} />
          <Float speed={2} rotationIntensity={2} floatIntensity={1.5}>
            <mesh>
              <coneGeometry args={[0.7, 1.5, 8]} />
              <meshStandardMaterial color="#3b82f6" emissive="#2563eb" emissiveIntensity={0.6} transparent opacity={0.35} />
            </mesh>
          </Float>
          ...
          <Float speed={1.6} rotationIntensity={1.5}>
            <mesh position={[-2.4, 1.2, -0.6]}>
              ...
              <meshStandardMaterial color="#60a5fa" emissive="#3b82f6" emissiveIntensity={0.55} transparent opacity={0.28} />
            </mesh>
          </Float>
          <Float speed={1.8} rotationIntensity={1.8}>
            <mesh position={[0, 2, -1]}>
              <coneGeometry args={[0.4, 0.8, 8]} />
              <meshStandardMaterial color="#fbbf24" emissive="#f59e0b" emissiveIntensity={0.5} transparent opacity={0.26} />
            </mesh>
          </Float>
          <OrbitControls enableZoom={false} enablePan={false} autoRotate autoRotateSpeed={0.4} />
        </Canvas>
      </Suspense>
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(60%_40%_at_50%_50%,rgba(59,130,246,.3),transparent),radial-gradient(50%_50%_at_80%_80%,rgba(245,158,11,.25),transparent),radial-gradient(40%_40%_at_20%_20%,rgba(96,165,250,.2),transparent)]" />
    </div>
  );
}

// --- Micro Simulations (Econ) ---
function SupplyDemandSim() {
  const [price, setPrice] = useState(50);
  const supply = price * 0.8;
  const demand = 100 - price * 0.6;
  const equilibrium = supply <= demand ? supply : demand;
  return (
    <div className="rounded-2xl bg-white/5 ring-1 ring-white/10 p-4">
      <div className="mb-2 flex items-center gap-2 text-sm text-white/80">
        <TrendingUp className="h-4 w-4 text-blue-300" /> Supply & Demand
      </div>
      <div className="text-xs text-white/60 mb-3">Price: ${price} → Q: {equilibrium.toFixed(0)}</div>
      <svg viewBox="0 0 100 50" className="w-full h-32">
        {/* Supply curve (upward) */}
        <path d="M 10 45 L 50 30 L 90 15" fill="none" stroke="#3b82f6" strokeWidth="2" />
        {/* Demand curve (downward) */}
        <path d="M 10 15 L 50 30 L 90 45" fill="none" stroke="#f59e0b" strokeWidth="2" />
        {/* Price line */}
        <line x1={price} x2={price} y1="0" y2="50" stroke="rgba(255,255,255,.3)" strokeWidth="1" strokeDasharray="2,2" />
        {/* Equilibrium point */}
        <circle cx="50" cy="30" r="2.5" fill="#60a5fa" />
        ...
        <text x="50" y="45" fontSize="8" fill="rgba(255,255,255,.6)" textAnchor="middle">S: {supply.toFixed(0)}, D: {demand.toFixed(0)}</text>
      </svg>
      <label className="text-xs text-white/70 block mt-2">Price ($)
        <input type="range" min={10} max={90} value={price} onChange={(e) => setPrice(+e.target.value)} className="w-full" />
      </label>
    </div>
  );
}
...
function CompoundInterestSim() {
  ...
  return (
    <div className="rounded-2xl bg-white/5 ring-1 ring-white/10 p-4">
      <div className="mb-2 flex items-center gap-2 text-sm text-white/80">
        <DollarSign className="h-4 w-4 text-amber-300" /> Compound Interest
      </div>
      <div className="text-xs text-white/60 mb-3">${principal} @ {rate}% for {years}yr = ${amount.toFixed(2)}</div>
      <svg viewBox="0 0 100 50" className="w-full h-32">
        {/* Bar chart showing growth */}
        <rect x="20" y={50 - (principal / 50)} width="15" fill="#3b82f6" opacity={0.7} />
        <rect x="40" y={50 - (amount / 50)} width="15" fill="#f59e0b" />
        <line x1="0" x2="100" y1="50" y2="50" stroke="rgba(255,255,255,.2)" strokeWidth="1" />
        <text x="27.5" y="45" fontSize="7" fill="rgba(255,255,255,.6)" textAnchor="middle">${principal}</text>
        <text x="47.5" y="45" fontSize="7" fill="rgba(255,255,255,.6)" textAnchor="middle">${amount.toFixed(0)}</text>
        <text x="50" y="35" fontSize="8" fill="rgba(255,255,255,.6)" textAnchor="middle">+${interest.toFixed(2)}</text>
      </svg>
      <div className="mt-3 grid grid-cols-3 gap-2">
        <label className="text-xs text-white/70">Principal ($)
          <input type="range" min={100} max={5000} step={100} value={principal} onChange={(e) => setPrincipal(+e.target.value)} className="w-full" />
        </label>
        <label className="text-xs text-white/70">Rate (%)
          <input type="range" min={1} max={20} value={rate} onChange={(e) => setRate(+e.target.value)} className="w-full" />
        </label>
        <label className="text-xs text-white/70">Years
...
function ResourceCard({ r, bookmarked, onBookmark }: { r: Resource; bookmarked?: boolean; onBookmark?: () => void }) {
  return (
    <motion.a href={r.link} target="_blank" rel="noreferrer" whileHover={{ y: -4 }}
      ...
      <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-blue-500/10 to-amber-500/10 opacity-0 transition-opacity group-hover:opacity-100" />
...
// --- Main Econ Hub ---
export default function EconomicsBusinessLearningHub() {
  const { discipline = "economics-business" } = useParams();
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        console.error("Error loading economics & business hub:", error);
        setLoading(false);
      }
    }
  ...
  useEffect(() => {
    ...
    const q = activeStage.quiz || [
      { q: `In "${activeStage.title}", what matters most?`, opts: ["Economic analysis & business strategy", "Memorization", "Fashion", "Speed typing"], correct: "Economic analysis & business strategy" },
      { q: "A key econ skill is…", opts: ["Market analysis & quantitative thinking", "Mime", "Crosswords", "Origami"], correct: "Market analysis & quantitative thinking" },
    ];
      ...
      <div className="relative min-h-screen">
        ...
        <div className="grid place-items-center min-h-screen">
          <div className="flex items-center gap-3 text-white/80">
            <Loader2 className="h-5 w-5 animate-spin" /> Loading your Economics & Business journey…
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="relative min-h-screen text-white">
      ...
      <header className="relative mx-auto max-w-6xl px-6 pt-14 text-center">
        <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={springy}>
          <div className="flex items-center justify-center gap-3 rounded-full bg-white/5 px-6 py-3 ring-1 ring-white/10 backdrop-blur mx-auto w-fit mb-6">
            ...
            <p className="font-semibold tracking-wide">Economics & Business Learning Hub</p>
          </div>
          <h1 className="text-balance bg-gradient-to-br from-white to-blue-200 bg-clip-text text-4xl sm:text-6xl font-extrabold text-transparent">
            Master markets, shape strategy, drive impact
          </h1>
          <p className="max-w-2xl mx-auto mt-3 text-white/80">
            A comprehensive business and economics journey for future leaders. Explore markets, finance, and strategic thinking — right here.
          </p>
          ...
          <div className="mt-6 relative grid place-items-center mx-auto w-fit">
            <svg width="120" height="120" viewBox="0 0 120 120" className="drop-shadow">
              ...
              <motion.circle
                ...
                stroke="url(#econgrad)" strokeLinecap="round"
              ...
              />
              <defs>
                <linearGradient id="econgrad" x1="0" x2="1" y1="0" y2="0">
          ...
          {hub.stages.map((stage: any, idx: number) => {
            ...
            return (
              <li key={stage.id} className="ml-6 pb-10">
                <span className="absolute -left-[9px] mt-1 grid h-4 w-4 place-items-center rounded-full bg-blue-400 ring-2 ring-blue-300/50" />
                <motion.div whileHover={{ y: -2 }} className="rounded-2xl bg-white/5 p-5 backdrop-blur ring-1 ring-white/10">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => setActiveStage(stage)} className="inline-flex items-center gap-1 rounded-full bg-blue-500/20 px-3 py-1 text-blue-200 ring-1 ring-blue-300/30 hover:bg-blue-500/30">
              ...
              <div className="grid gap-6 p-6 sm:grid-cols-5">
                ...
                <div className="sm:col-span-3 space-y-6">
                  ...
                  <div className="rounded-2xl bg-black/30 p-4 ring-1 ring-white/10 backdrop-blur-md">
                    <h4 className="mb-3 font-semibold text-blue-200">Checklist</h4>
                  ...
                  <div className="rounded-2xl bg-black/30 p-4 ring-1 ring-white/10 backdrop-blur-md">
                    <h4 className="mb-3 font-semibold text-blue-200">Curated resources</h4>
                  ...
                  <div className="rounded-2xl bg-black/30 p-4 ring-1 ring-white/10 backdrop-blur-md">
                    <h4 className="mb-3 font-semibold text-blue-200">Reflection</h4>
                    <textarea
                      ...
                      placeholder="What markets, strategies, or insights did you explore?"
                ...
                <div className="sm:col-span-2 flex flex-col gap-4">
                  <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                    <div className="mb-2 flex items-center gap-2 text-sm text-white/80"><Sparkles className="h-4 w-4 text-blue-300" /> Market Playground</div>
                  ...
                  <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                    <div className="mb-2 flex items-center gap-2 text-sm text-white/80"><GraduationCap className="h-4 w-4 text-blue-300" /> Quick quiz</div>
                    <div className="space-y-3">
                      {(activeStage.quiz || [
                        { q: `In "${activeStage.title}" what matters most?`, opts: ["Economic analysis & business strategy", "Memorization", "Graphic design", "Speed typing"], correct: "Economic analysis & business strategy" },
                        { q: "A key econ skill is…", opts: ["Market analysis & quantitative thinking", "Mime", "Crosswords", "Origami"], correct: "Market analysis & quantitative thinking" },
                      ]).map((Q: any, i: number) => (
                        <div key={i}>
                            ...
                            {Q.opts.map((opt: string) => (
                              <button key={opt} onClick={() => setQuizAnswers((prev) => ({ ...prev, [i]: opt }))}
                                className={cx("rounded-full border border-white/15 px-3 py-1 text-sm",
                                  quizAnswers[i] === opt ? "bg-blue-500/30" : "bg-white/5 hover:bg-white/10")}>{opt}</button>
                            ))}
                          </div>
                        </div>
                      ))}
                  ...
                  {(activeStage.regions?.Global?.projectIdeas?.length > 0) && (
                    <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                      <div className="mb-2 flex items-center gap-2 text-sm text-white/80"><TrendingUp className="h-4 w-4 text-amber-300" /> Project ideas</div>
                      <div className="flex snap-x snap-mandatory overflow-x-auto gap-3 pb-2">
                        {activeStage.regions.Global.projectIdeas.map((idea: string, i: number) => (
                          <motion.div key={i} whileHover={{ y: -2 }} className="snap-start min-w-[280px] rounded-2xl bg-white/5 p-4 ring-1 ring-white/10 backdrop-blur">
                            <div className="mb-2 flex items-center gap-2 text-xs text-white/70"><Sparkles className="h-4 w-4 text-blue-200" /> Idea</div>
      ...
      <footer className="py-10 text-center text-xs text-white/50">Made for future business leaders 💼</footer>
    </div>
  );
}
