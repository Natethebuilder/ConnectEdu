...
// 3D data/analytics background (emerald + green data science theme)
function DataScienceBG() {
  return (
...
// --- Micro Simulations (DS) ---
function ScatterPlotSim() {
  const [correlation, setCorrelation] = useState(0.7);
  const points = useMemo(() => {
    const pts: [number, number][] = [];
    for (let i = 0; i < 20; i++) {
      const x = 10 + (i / 19) * 80;
      const baseY = 25 + (i / 19) * 20;
      const noise = (Math.random() - 0.5) * (1 - correlation) * 15;
      const y = Math.max(5, Math.min(45, baseY + noise));
      pts.push([x, y]);
    }
    return pts;
  }, [correlation]);
  return (
    <div className="rounded-2xl bg-white/5 ring-1 ring-white/10 p-4">
      <div className="mb-2 flex items-center gap-2 text-sm text-white/80">
        <BarChart3 className="h-4 w-4 text-emerald-300" /> Scatter Plot
      </div>
      <div className="text-xs text-white/60 mb-3">Correlation: {(correlation * 100).toFixed(0)}%</div>
      <svg viewBox="0 0 100 50" className="w-full h-32">
        {points.map((p, i) => (
          <circle key={i} cx={p[0]} cy={p[1]} r="1.5" fill="#10b981" opacity={0.7} />
        ))}
      ...
      <label className="text-xs text-white/70 block mt-2">Correlation
        <input type="range" min={0} max={1} step={0.1} value={correlation} onChange={(e) => setCorrelation(+e.target.value)} className="w-full" />
      </label>
    </div>
  );
}

function RegressionLineSim() {
  const [slope, setSlope] = useState(0.5);
  const [intercept, setIntercept] = useState(10);
  const points = useMemo(() => {
    const pts: [number, number][] = [];
    for (let x = 10; x <= 90; x += 5) {
      const y = intercept + slope * (x - 50);
      pts.push([x, Math.max(5, Math.min(45, 50 - y))]);
    }
    return pts;
  }, [slope, intercept]);
  ...
  return (
    <div className="rounded-2xl bg-white/5 ring-1 ring-white/10 p-4">
      <div className="mb-2 flex items-center gap-2 text-sm text-white/80">
        <TrendingUp className="h-4 w-4 text-green-300" /> Regression Line
      </div>
      <div className="text-xs text-white/60 mb-3">y = {slope.toFixed(2)}x + {intercept.toFixed(1)}</div>
      <svg viewBox="0 0 100 50" className="w-full h-32">
        {d && <path d={d} fill="none" stroke="#10b981" strokeWidth="2" />}
        <line x1="0" x2="100" y1="50" y2="50" stroke="rgba(255,255,255,.2)" strokeWidth="1" />
        <line x1="50" y1="0" x2="50" y2="50" stroke="rgba(255,255,255,.2)" strokeWidth="1" />
        <text x="50" y="45" fontSize="8" fill="rgba(255,255,255,.6)" textAnchor="middle">y = {slope.toFixed(2)}x + {intercept.toFixed(1)}</text>
      </svg>
      <div className="mt-3 grid grid-cols-2 gap-3">
        <label className="text-xs text-white/70">Slope
          <input type="range" min={-1} max={1} step={0.1} value={slope} onChange={(e) => setSlope(+e.target.value)} className="w-full" />
        </label>
        <label className="text-xs text-white/70">Intercept
          <input type="range" min={0} max={20} value={intercept} onChange={(e) => setIntercept(+e.target.value)} className="w-full" />
        </label>
      </div>
    </div>
  );
...
// --- Main DS Hub ---
export default function DataScienceLearningHub() {
  const { discipline = "data-science" } = useParams();
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        console.error("Error loading data science hub:", error);
        setLoading(false);
      }
    }
  ...
  useEffect(() => {
    ...
    const q = activeStage.quiz || [
      { q: `In "${activeStage.title}", what matters most?`, opts: ["Data analysis & machine learning", "Memorization", "Fashion", "Speed typing"], correct: "Data analysis & machine learning" },
      { q: "A key DS skill is…", opts: ["Statistical thinking & programming", "Mime", "Crosswords", "Origami"], correct: "Statistical thinking & programming" },
    ];
      ...
      <div className="relative min-h-screen">
        ...
        <div className="grid place-items-center min-h-screen">
          <div className="flex items-center gap-3 text-white/80">
            <Loader2 className="h-5 w-5 animate-spin" /> Loading your Data Science journey…
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="relative min-h-screen text-white">
      ...
      <header className="relative mx-auto max-w-6xl px-6 pt-14 text-center">
        <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={springy}>
          <div className="flex items-center justify-center gap-3 rounded-full bg-white/5 px-6 py-3 ring-1 ring-white/10 backdrop-blur mx-auto w-fit mb-6">
            ...
            <p className="font-semibold tracking-wide">Data Science Learning Hub</p>
          </div>
          <h1 className="text-balance bg-gradient-to-br from-white to-emerald-200 bg-clip-text text-4xl sm:text-6xl font-extrabold text-transparent">
            Turn data into insights, insights into impact
          </h1>
          <p className="max-w-2xl mx-auto mt-3 text-white/80">
            A comprehensive data science journey for aspiring analysts and ML engineers. Explore statistics, algorithms, and real-world applications — right here.
          </p>
          ...
          <div className="mt-6 relative grid place-items-center mx-auto w-fit">
            <svg width="120" height="120" viewBox="0 0 120 120" className="drop-shadow">
              ...
              <motion.circle
                ...
                stroke="url(#dsgrad)" strokeLinecap="round"
              ...
              />
              <defs>
                <linearGradient id="dsgrad" x1="0" x2="1" y1="0" y2="0">
              ...
              <div className="grid gap-6 p-6 sm:grid-cols-5">
                ...
                <div className="sm:col-span-3 space-y-6">
                  ...
                  <div className="rounded-2xl bg-black/30 p-4 ring-1 ring-white/10 backdrop-blur-md">
                    ...
                    <textarea
                      ...
                      placeholder="What datasets, models, or insights did you explore?"
                ...
                <div className="sm:col-span-2 flex flex-col gap-4">
                  <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                    <div className="mb-2 flex items-center gap-2 text-sm text-white/80"><Sparkles className="h-4 w-4 text-emerald-300" /> Data Playground</div>
                  ...
                  <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                    ...
                    <div className="space-y-3">
                      {(activeStage.quiz || [
                        { q: `In "${activeStage.title}" what matters most?`, opts: ["Data analysis & machine learning", "Memorization", "Graphic design", "Speed typing"], correct: "Data analysis & machine learning" },
                        { q: "A key DS skill is…", opts: ["Statistical thinking & programming", "Mime", "Crosswords", "Origami"], correct: "Statistical thinking & programming" },
                      ]).map((Q: any, i: number) => (
                        <div key={i}>
                  ...
                  {(activeStage.regions?.Global?.projectIdeas?.length > 0) && (
                    <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                      <div className="mb-2 flex items-center gap-2 text-sm text-white/80"><Database className="h-4 w-4 text-green-300" /> Project ideas</div>
      ...
      <footer className="py-10 text-center text-xs text-white/50">Made for future data scientists 📊</footer>
    </div>
  );
}
