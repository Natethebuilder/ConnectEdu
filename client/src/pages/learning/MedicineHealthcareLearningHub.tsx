...
// 3D medicine/healthcare background (red + pink theme)
function MedicineBG() {
  return (
    <div className="absolute inset-0 -z-10">
      <Suspense fallback={<div className="absolute inset-0 bg-[#1a0a0a]" />}>
        <Canvas camera={{ position: [0, 0, 6] }} dpr={[1, 2]}>
          <color attach="background" args={["#1a0a0a"]} />
          <ambientLight intensity={0.3} />
          <pointLight position={[5, 5, 5]} color="#dc2626" intensity={1.1} />
          <pointLight position={[-5, -3, 3]} color="#ec4899" intensity={0.9} />
          <Float speed={2} rotationIntensity={2} floatIntensity={1.5}>
            <mesh>
              <torusKnotGeometry args={[0.6, 0.2, 64, 16]} />
              <meshStandardMaterial color="#dc2626" emissive="#b91c1c" emissiveIntensity={0.6} transparent opacity={0.35} />
            </mesh>
          </Float>
          <Float speed={2.4} rotationIntensity={2}>
            <mesh position={[2.2, -1.3, 0]}>
              <torusKnotGeometry args={[0.5, 0.18, 64, 16]} />
              <meshStandardMaterial color="#ec4899" emissive="#db2777" emissiveIntensity={0.6} transparent opacity={0.3} />
            </mesh>
          </Float>
          <Float speed={1.6} rotationIntensity={1.5}>
            <mesh position={[-2.4, 1.2, -0.6]}>
              <torusKnotGeometry args={[0.4, 0.15, 64, 16]} />
              <meshStandardMaterial color="#f87171" emissive="#dc2626" emissiveIntensity={0.55} transparent opacity={0.28} />
            </mesh>
          </Float>
          <Float speed={1.8} rotationIntensity={1.8}>
            <mesh position={[0, 2, -1]}>
              <torusKnotGeometry args={[0.35, 0.12, 64, 16]} />
              <meshStandardMaterial color="#f472b6" emissive="#ec4899" emissiveIntensity={0.5} transparent opacity={0.26} />
            </mesh>
          </Float>
          <OrbitControls enableZoom={false} enablePan={false} autoRotate autoRotateSpeed={0.4} />
        </Canvas>
      </Suspense>
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(60%_40%_at_50%_50%,rgba(220,38,38,.3),transparent),radial-gradient(50%_50%_at_80%_80%,rgba(236,72,153,.25),transparent),radial-gradient(40%_40%_at_20%_20%,rgba(248,113,113,.2),transparent)]" />
    </div>
  );
}

// --- Micro Simulations (Medicine) ---
function HeartRateSim() {
  ...
  const zone = bpm < targetZone.min ? "Rest" : bpm > targetZone.max ? "High" : "Target";
  const zoneColor = zone === "Target" ? "#10b981" : zone === "High" ? "#ef4444" : "#64748b";
  
  // Generate ECG-like waveform
  const points: [number, number][] = [];
  const centerY = 25;
  for (let x = 0; x <= 100; x += 1) {
    const cycle = (x / 100) * (bpm / 60) * 4; // 4 cycles for visibility
    let y = centerY;
    if (cycle % 1 < 0.1) {
      // P wave
      y = centerY - 2;
    } else if (cycle % 1 < 0.15) {
      // QRS complex (spike)
      y = centerY - 8;
    } else if (cycle % 1 < 0.2) {
      // Return to baseline
  ...
  return (
    <div className="rounded-2xl bg-white/5 ring-1 ring-white/10 p-4">
      <div className="mb-2 flex items-center gap-2 text-sm text-white/80">
        <Activity className="h-4 w-4 text-red-300" /> Heart Rate Monitor
      </div>
      <div className="text-xs text-white/60 mb-3">{bpm} BPM, Age: {age} → Zone: {zone} ({targetZone.min}-{targetZone.max} BPM)</div>
      <svg viewBox="0 0 100 50" className="w-full h-32">
        {/* ECG waveform */}
        <path d={`M ${points.map(p => `${p[0]},${p[1]}`).join(" L ")}`} fill="none" stroke={zoneColor} strokeWidth="1.5" />
        {/* Baseline */}
        <line x1="0" x2="100" y1={centerY} y2={centerY} stroke="rgba(255,255,255,.2)" strokeWidth="0.5" strokeDasharray="2,2" />
        {/* Zone indicators */}
        <line x1="0" x2="100" y1="15" y2="15" stroke="rgba(16,185,129,.3)" strokeWidth="1" strokeDasharray="1,1" />
        <line x1="0" x2="100" y1="35" y2="35" stroke="rgba(16,185,129,.3)" strokeWidth="1" strokeDasharray="1,1" />
        <text x="2" y="13" fontSize="6" fill="rgba(255,255,255,.6)">Max</text>
        <text x="2" y="37" fontSize="6" fill="rgba(255,255,255,.6)">Min</text>
      </svg>
      <div className="mt-3 grid grid-cols-2 gap-2">
        <label className="text-xs text-white/70">Heart Rate (BPM)
          <input type="range" min={40} max={200} value={bpm} onChange={(e) => setBpm(+e.target.value)} className="w-full" />
        </label>
        <label className="text-xs text-white/70">Age
          <input type="range" min={18} max={80} value={age} onChange={(e) => setAge(+e.target.value)} className="w-full" />
        </label>
      </div>
      <div className="mt-2 text-xs text-white/60">Target zone: 50-85% of max HR ({targetZone.min}-{targetZone.max} BPM)</div>
    </div>
  );
}

function DosageCalcSim() {
  const [weight, setWeight] = useState(70);
  const [dosePerKg, setDosePerKg] = useState(5);
  const totalDose = weight * dosePerKg;
  const tabletStrength = 250; // mg per tablet
  const tablets = Math.ceil(totalDose / tabletStrength);
  
  return (
    <div className="rounded-2xl bg-white/5 ring-1 ring-white/10 p-4">
      <div className="mb-2 flex items-center gap-2 text-sm text-white/80">
        <Stethoscope className="h-4 w-4 text-pink-300" /> Dosage Calculator
      </div>
      <div className="text-xs text-white/60 mb-3">Weight: {weight}kg, Dose: {dosePerKg}mg/kg → Total: {totalDose}mg ({tablets} tablets @ {tabletStrength}mg)</div>
      <svg viewBox="0 0 100 50" className="w-full h-32">
        {/* Visual representation */}
        <rect x="10" y="15" width="20" height="20" fill="#dc2626" opacity={0.7} rx="2" />
        <text x="20" y="28" fontSize="7" fill="rgba(255,255,255,.8)" textAnchor="middle" fontWeight="bold">{weight}kg</text>
        <text x="20" y="38" fontSize="6" fill="rgba(255,255,255,.6)" textAnchor="middle">Weight</text>
        
        <text x="40" y="30" fontSize="8" fill="rgba(255,255,255,.8)">×</text>
        
        <rect x="50" y="15" width="20" height="20" fill="#ec4899" opacity={0.7} rx="2" />
        <text x="60" y="28" fontSize="7" fill="rgba(255,255,255,.8)" textAnchor="middle" fontWeight="bold">{dosePerKg}mg/kg</text>
        <text x="60" y="38" fontSize="6" fill="rgba(255,255,255,.6)" textAnchor="middle">Dose</text>
        
        <text x="75" y="30" fontSize="8" fill="rgba(255,255,255,.8)">=</text>
        
        <rect x="82" y="12" width="12" height="26" fill="#10b981" opacity={0.8} rx="2" />
        <text x="88" y="28" fontSize="7" fill="rgba(255,255,255,.9)" textAnchor="middle" fontWeight="bold">{totalDose}mg</text>
        <text x="88" y="40" fontSize="6" fill="rgba(255,255,255,.7)" textAnchor="middle">Total</text>
      </svg>
      <div className="mt-3 grid grid-cols-2 gap-2">
        <label className="text-xs text-white/70">Weight (kg)
          <input type="range" min={10} max={150} value={weight} onChange={(e) => setWeight(+e.target.value)} className="w-full" />
        </label>
        <label className="text-xs text-white/70">Dose (mg/kg)
          <input type="range" min={1} max={20} value={dosePerKg} onChange={(e) => setDosePerKg(+e.target.value)} className="w-full" />
        </label>
      </div>
      <div className="mt-2 text-xs text-white/60">Tablets needed: {tablets} × {tabletStrength}mg = {tablets * tabletStrength}mg</div>
    </div>
  );
}
...
function ResourceCard({ r, bookmarked, onBookmark }: { r: Resource; bookmarked?: boolean; onBookmark?: () => void }) {
  return (
    <motion.a href={r.link} target="_blank" rel="noreferrer" whileHover={{ y: -4 }}
      ...
      <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-red-500/10 to-pink-500/10 opacity-0 transition-opacity group-hover:opacity-100" />
...
// --- Main Medicine & Healthcare Hub ---
export default function MedicineHealthcareLearningHub() {
  const { discipline = "medicine-healthcare" } = useParams();
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        console.error("Error loading medicine & healthcare hub:", error);
        setLoading(false);
      }
    }
  ...
  useEffect(() => {
    ...
    const q = activeStage.quiz || [
      { q: `In "${activeStage.title}", what matters most?`, opts: ["Science foundations & clinical experience", "Memorization", "Fashion", "Speed typing"], correct: "Science foundations & clinical experience" },
      { q: "A key med skill is…", opts: ["Patient care & scientific knowledge", "Mime", "Crosswords", "Origami"], correct: "Patient care & scientific knowledge" },
    ];
      ...
      <div className="relative min-h-screen">
        ...
        <div className="grid place-items-center min-h-screen">
          <div className="flex items-center gap-3 text-white/80">
            <Loader2 className="h-5 w-5 animate-spin" /> Loading your Medicine & Healthcare journey…
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="relative min-h-screen text-white">
      ...
      <header className="relative mx-auto max-w-6xl px-6 pt-14 text-center">
        <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={springy}>
          <div className="flex items-center justify-center gap-3 rounded-full bg-white/5 px-6 py-3 ring-1 ring-white/10 backdrop-blur mx-auto w-fit mb-6">
            ...
            <p className="font-semibold tracking-wide">Medicine & Healthcare Learning Hub</p>
          </div>
          <h1 className="text-balance bg-gradient-to-br from-white to-red-200 bg-clip-text text-4xl sm:text-6xl font-extrabold text-transparent">
            Heal, care, advance medical science
          </h1>
          <p className="max-w-2xl mx-auto mt-3 text-white/80">
            A comprehensive medicine and healthcare journey for future doctors, nurses, and healthcare professionals. Master patient care and medical science — right here.
          </p>
          ...
          <div className="mt-6 relative grid place-items-center mx-auto w-fit">
            <svg width="120" height="120" viewBox="0 0 120 120" className="drop-shadow">
              ...
              <motion.circle
                ...
                stroke="url(#medgrad)" strokeLinecap="round"
              ...
              />
              <defs>
                <linearGradient id="medgrad" x1="0" x2="1" y1="0" y2="0">
          ...
          {hub.stages.map((stage: any, idx: number) => {
            ...
            return (
              <li key={stage.id} className="ml-6 pb-10">
                <span className="absolute -left-[9px] mt-1 grid h-4 w-4 place-items-center rounded-full bg-red-400 ring-2 ring-red-300/50" />
                <motion.div whileHover={{ y: -2 }} className="rounded-2xl bg-white/5 p-5 backdrop-blur ring-1 ring-white/10">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => setActiveStage(stage)} className="inline-flex items-center gap-1 rounded-full bg-red-500/20 px-3 py-1 text-red-200 ring-1 ring-red-300/30 hover:bg-red-500/30">
              ...
              <div className="grid gap-6 p-6 sm:grid-cols-5">
                ...
                <div className="sm:col-span-3 space-y-6">
                  ...
                  <div className="rounded-2xl bg-black/30 p-4 ring-1 ring-white/10 backdrop-blur-md">
                    <h4 className="mb-3 font-semibold text-red-200">Checklist</h4>
                  ...
                  <div className="rounded-2xl bg-black/30 p-4 ring-1 ring-white/10 backdrop-blur-md">
                    <h4 className="mb-3 font-semibold text-red-200">Curated resources</h4>
                  ...
                  <div className="rounded-2xl bg-black/30 p-4 ring-1 ring-white/10 backdrop-blur-md">
                    <h4 className="mb-3 font-semibold text-red-200">Reflection</h4>
                    <textarea
                      ...
                      placeholder="What medical concepts, clinical skills, or healthcare insights did you explore?"
                ...
                <div className="sm:col-span-2 flex flex-col gap-4">
                  <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                    <div className="mb-2 flex items-center gap-2 text-sm text-white/80"><Sparkles className="h-4 w-4 text-red-300" /> Medical Playground</div>
                  ...
                  <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                    <div className="mb-2 flex items-center gap-2 text-sm text-white/80"><GraduationCap className="h-4 w-4 text-red-300" /> Quick quiz</div>
                    <div className="space-y-3">
                      {(activeStage.quiz || [
                        { q: `In "${activeStage.title}" what matters most?`, opts: ["Science foundations & clinical experience", "Memorization", "Graphic design", "Speed typing"], correct: "Science foundations & clinical experience" },
                        { q: "A key med skill is…", opts: ["Patient care & scientific knowledge", "Mime", "Crosswords", "Origami"], correct: "Patient care & scientific knowledge" },
                      ]).map((Q: any, i: number) => (
                        <div key={i}>
                            ...
                            {Q.opts.map((opt: string) => (
                              <button key={opt} onClick={() => setQuizAnswers((prev) => ({ ...prev, [i]: opt }))}
                                className={cx("rounded-full border border-white/15 px-3 py-1 text-sm",
                                  quizAnswers[i] === opt ? "bg-red-500/30" : "bg-white/5 hover:bg-white/10")}>{opt}</button>
                            ))}
                          </div>
                        </div>
                      ))}
                  ...
                  {(activeStage.regions?.Global?.projectIdeas?.length > 0) && (
                    <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                      <div className="mb-2 flex items-center gap-2 text-sm text-white/80"><Heart className="h-4 w-4 text-pink-300" /> Project ideas</div>
                      <div className="flex snap-x snap-mandatory overflow-x-auto gap-3 pb-2">
                        {activeStage.regions.Global.projectIdeas.map((idea: string, i: number) => (
                          <motion.div key={i} whileHover={{ y: -2 }} className="snap-start min-w-[280px] rounded-2xl bg-white/5 p-4 ring-1 ring-white/10 backdrop-blur">
                            <div className="mb-2 flex items-center gap-2 text-xs text-white/70"><Sparkles className="h-4 w-4 text-red-200" /> Idea</div>
      ...
      <footer className="py-10 text-center text-xs text-white/50">Made for future healthcare professionals ❤️</footer>
    </div>
  );
}
