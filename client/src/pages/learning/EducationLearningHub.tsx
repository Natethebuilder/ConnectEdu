...
// 3D education/teaching background (teal + cyan theme)
function EducationBG() {
  return (
    <div className="absolute inset-0 -z-10">
      <Suspense fallback={<div className="absolute inset-0 bg-[#0a1a1a]" />}>
...
// --- Micro Simulations (Education) ---
function LearningProgressSim() {
  const [weeks, setWeeks] = useState(4);
  // Learning curve: progress = 100 * (1 - e^(-weeks/rate))
  const rate = 3;
  const progress = 100 * (1 - Math.exp(-weeks / rate));
  const points: [number, number][] = [];
  for (let w = 0; w <= 12; w++) {
    const p = 100 * (1 - Math.exp(-w / rate));
    points.push([(w / 12) * 100, 50 - (p / 100) * 40]);
  }
  return (
    <div className="rounded-2xl bg-white/5 ring-1 ring-white/10 p-4">
      <div className="mb-2 flex items-center gap-2 text-sm text-white/80">
        <BookOpen className="h-4 w-4 text-teal-300" /> Learning Progress
      </div>
      <div className="text-xs text-white/60 mb-3">After {weeks} weeks: {progress.toFixed(1)}% mastery</div>
      <svg viewBox="0 0 100 50" className="w-full h-32">
        <path d={`M ${points.map(p => `${p[0]},${p[1]}`).join(" L ")}`} fill="none" stroke="#14b8a6" strokeWidth="2" />
        <circle cx={(weeks / 12) * 100} cy={50 - (progress / 100) * 40} r="2.5" fill="#06b6d4" />
        ...
        <text x="50" y="45" fontSize="7" fill="rgba(255,255,255,.6)" textAnchor="middle">Weeks</text>
        <text x="5" y="10" fontSize="7" fill="rgba(255,255,255,.6)">100%</text>
        <text x="5" y="48" fontSize="7" fill="rgba(255,255,255,.6)">0%</text>
      </svg>
      <label className="text-xs text-white/70 block mt-2">Weeks of study
        <input type="range" min={0} max={12} value={weeks} onChange={(e) => setWeeks(+e.target.value)} className="w-full" />
      </label>
    </div>
  );
}

function StudentEngagementSim() {
  const [engagement, setEngagement] = useState(70);
  // Higher engagement → better outcomes
  const comprehension = Math.min(100, engagement * 1.2);
  const retention = Math.min(100, engagement * 0.9);
  return (
    <div className="rounded-2xl bg-white/5 ring-1 ring-white/10 p-4">
      <div className="mb-2 flex items-center gap-2 text-sm text-white/80">
        <Users className="h-4 w-4 text-cyan-300" /> Student Engagement
      </div>
      <div className="text-xs text-white/60 mb-3">{engagement}% engagement → {comprehension.toFixed(0)}% comprehension, {retention.toFixed(0)}% retention</div>
      <svg viewBox="0 0 100 50" className="w-full h-32">
        {/* Engagement bar */}
        <rect x="10" y="10" width="80" height="8" fill="rgba(255,255,255,.1)" rx="2" />
        <rect x="10" y="10" width={(engagement / 100) * 80} height="8" fill="#14b8a6" rx="2" />
        <text x="50" y="17" fontSize="7" fill="rgba(255,255,255,.8)" textAnchor="middle" fontWeight="bold">{engagement}%</text>
        {/* Comprehension bar */}
        <rect x="10" y="22" width="80" height="8" fill="rgba(255,255,255,.1)" rx="2" />
        <rect x="10" y="22" width={(comprehension / 100) * 80} height="8" fill="#06b6d4" rx="2" />
        <text x="50" y="29" fontSize="7" fill="rgba(255,255,255,.8)" textAnchor="middle">Comprehension: {comprehension.toFixed(0)}%</text>
        {/* Retention bar */}
        <rect x="10" y="34" width="80" height="8" fill="rgba(255,255,255,.1)" rx="2" />
        <rect x="10" y="34" width={(retention / 100) * 80} height="8" fill="#2dd4bf" rx="2" />
        <text x="50" y="41" fontSize="7" fill="rgba(255,255,255,.8)" textAnchor="middle">Retention: {retention.toFixed(0)}%</text>
      </svg>
      <label className="text-xs text-white/70 block mt-2">Engagement level (%)
        <input type="range" min={0} max={100} value={engagement} onChange={(e) => setEngagement(+e.target.value)} className="w-full" />
      </label>
      <div className="mt-2 text-xs text-white/60">Higher engagement leads to better learning outcomes</div>
    </div>
  );
}
...
export default function EducationLearningHub() {
  const { discipline = "education" } = useParams();
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        console.error("Error loading education hub:", error);
        setLoading(false);
      }
    }
  ...
  useEffect(() => {
    ...
    const q = activeStage.quiz || [
      { q: `In "${activeStage.title}", what matters most?`, opts: ["Teaching methods & educational impact", "Memorization", "Fashion", "Speed typing"], correct: "Teaching methods & educational impact" },
      { q: "A key education skill is…", opts: ["Pedagogy & student engagement", "Mime", "Crosswords", "Origami"], correct: "Pedagogy & student engagement" },
    ];
      ...
      <div className="relative min-h-screen">
        ...
        <div className="grid place-items-center min-h-screen">
          <div className="flex items-center gap-3 text-white/80">
            <Loader2 className="h-5 w-5 animate-spin" /> Loading your Education journey…
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="relative min-h-screen text-white">
      ...
      <header className="relative mx-auto max-w-6xl px-6 pt-14 text-center">
        <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={springy}>
          <div className="flex items-center justify-center gap-3 rounded-full bg-white/5 px-6 py-3 ring-1 ring-white/10 backdrop-blur mx-auto w-fit mb-6">
            ...
            <p className="font-semibold tracking-wide">Education Learning Hub</p>
          </div>
          <h1 className="text-balance bg-gradient-to-br from-white to-teal-200 bg-clip-text text-4xl sm:text-6xl font-extrabold text-transparent">
            Shape minds, transform futures, inspire learning
          </h1>
          <p className="max-w-2xl mx-auto mt-3 text-white/80">
            A comprehensive education journey for future teachers, administrators, and educational researchers. Master pedagogy and policy — right here.
          </p>
          ...
          <div className="mt-6 relative grid place-items-center mx-auto w-fit">
            <svg width="120" height="120" viewBox="0 0 120 120" className="drop-shadow">
              ...
              <motion.circle
                ...
                stroke="url(#edugrad)" strokeLinecap="round"
              ...
              />
              <defs>
                <linearGradient id="edugrad" x1="0" x2="1" y1="0" y2="0">
              ...
              <div className="grid gap-6 p-6 sm:grid-cols-5">
                ...
                <div className="sm:col-span-3 space-y-6">
                  ...
                  <div className="rounded-2xl bg-black/30 p-4 ring-1 ring-white/10 backdrop-blur-md">
                    ...
                    <textarea
                      ...
                      placeholder="What teaching methods, educational policies, or insights did you explore?"
                ...
                <div className="sm:col-span-2 flex flex-col gap-4">
                  <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                    <div className="mb-2 flex items-center gap-2 text-sm text-white/80"><Sparkles className="h-4 w-4 text-teal-300" /> Teaching Playground</div>
                    <div className="grid gap-3">
                      <LearningProgressSim />
                      <StudentEngagementSim />
                    </div>
                  </div>
                  ...
                  <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                    ...
                    <div className="space-y-3">
                      {(activeStage.quiz || [
                        { q: `In "${activeStage.title}" what matters most?`, opts: ["Teaching methods & educational impact", "Memorization", "Graphic design", "Speed typing"], correct: "Teaching methods & educational impact" },
                        { q: "A key education skill is…", opts: ["Pedagogy & student engagement", "Mime", "Crosswords", "Origami"], correct: "Pedagogy & student engagement" },
                      ]).map((Q: any, i: number) => (
                        <div key={i}>
                  ...
                  {(activeStage.regions?.Global?.projectIdeas?.length > 0) && (
                    <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                      <div className="mb-2 flex items-center gap-2 text-sm text-white/80"><BookOpen className="h-4 w-4 text-cyan-300" /> Project ideas</div>
      ...
      <footer className="py-10 text-center text-xs text-white/50">Made for future educators 📚</footer>
    </div>
  );
}
