...
// 3D history/literature background (amber + sepia theme)
function HistoryLiteratureBG() {
  return (
    <div className="absolute inset-0 -z-10">
      <Suspense fallback={<div className="absolute inset-0 bg-[#1a150f]" />}>
        <Canvas camera={{ position: [0, 0, 6] }} dpr={[1, 2]}>
          <color attach="background" args={["#1a150f"]} />
          <ambientLight intensity={0.3} />
          <pointLight position={[5, 5, 5]} color="#d97706" intensity={1.1} />
          <pointLight position={[-5, -3, 3]} color="#92400e" intensity={0.9} />
          <Float speed={2} rotationIntensity={2} floatIntensity={1.5}>
            <mesh>
              <tetrahedronGeometry args={[0.7, 0]} />
              <meshStandardMaterial color="#d97706" emissive="#b45309" emissiveIntensity={0.6} transparent opacity={0.35} />
            </mesh>
          </Float>
          <Float speed={2.4} rotationIntensity={2}>
            <mesh position={[2.2, -1.3, 0]}>
              <tetrahedronGeometry args={[0.6, 0]} />
              <meshStandardMaterial color="#92400e" emissive="#78350f" emissiveIntensity={0.6} transparent opacity={0.3} />
            </mesh>
          </Float>
          <Float speed={1.6} rotationIntensity={1.5}>
            <mesh position={[-2.4, 1.2, -0.6]}>
              <tetrahedronGeometry args={[0.5, 0]} />
              <meshStandardMaterial color="#f59e0b" emissive="#d97706" emissiveIntensity={0.55} transparent opacity={0.28} />
            </mesh>
          </Float>
          <Float speed={1.8} rotationIntensity={1.8}>
            <mesh position={[0, 2, -1]}>
              <tetrahedronGeometry args={[0.4, 0]} />
              <meshStandardMaterial color="#fbbf24" emissive="#d97706" emissiveIntensity={0.5} transparent opacity={0.26} />
            </mesh>
          </Float>
          <OrbitControls enableZoom={false} enablePan={false} autoRotate autoRotateSpeed={0.4} />
        </Canvas>
      </Suspense>
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(60%_40%_at_50%_50%,rgba(217,119,6,.3),transparent),radial-gradient(50%_50%_at_80%_80%,rgba(146,64,14,.25),transparent),radial-gradient(40%_40%_at_20%_20%,rgba(245,158,11,.2),transparent)]" />
    </div>
  );
}

// --- Micro Simulations (History/Lit) ---
function TimelineSim() {
  const [year, setYear] = useState(1500);
  const [focus, setFocus] = useState<"ancient" | "medieval" | "modern">("medieval");
  
  // Historical periods
  const periods = {
    ancient: { start: -3000, end: 500, label: "Ancient" },
    medieval: { start: 500, end: 1500, label: "Medieval" },
    modern: { start: 1500, end: 2000, label: "Modern" },
  };
  
  const currentPeriod = year < periods.medieval.start ? "ancient" : year < periods.modern.start ? "medieval" : "modern";
  const periodData = periods[currentPeriod];
  const progress = ((year - periodData.start) / (periodData.end - periodData.start)) * 100;
  
  return (
    <div className="rounded-2xl bg-white/5 ring-1 ring-white/10 p-4">
      <div className="mb-2 flex items-center gap-2 text-sm text-white/80">
        <Scroll className="h-4 w-4 text-amber-300" /> Historical Timeline
      </div>
      <div className="text-xs text-white/60 mb-3">Year: {year > 0 ? year : `${Math.abs(year)} BCE`} ({periodData.label} Period)</div>
      <svg viewBox="0 0 100 50" className="w-full h-32">
        {/* Timeline bar */}
        <rect x="10" y="20" width="80" height="8" fill="rgba(255,255,255,.1)" rx="2" />
        <rect x="10" y="20" width={(progress / 100) * 80} height="8" fill="#d97706" rx="2" />
        {/* Period markers */}
        <line x1="10" y1="15" x2="10" y2="35" stroke="rgba(255,255,255,.3)" strokeWidth="1" />
        <line x1="50" y1="15" x2="50" y2="35" stroke="rgba(255,255,255,.3)" strokeWidth="1" />
        <line x1="90" y1="15" x2="90" y2="35" stroke="rgba(255,255,255,.3)" strokeWidth="1" />
        <text x="10" y="12" fontSize="6" fill="rgba(255,255,255,.6)" textAnchor="middle">Ancient</text>
        <text x="50" y="12" fontSize="6" fill="rgba(255,255,255,.6)" textAnchor="middle">Medieval</text>
        <text x="90" y="12" fontSize="6" fill="rgba(255,255,255,.6)" textAnchor="middle">Modern</text>
        {/* Current position marker */}
        <circle cx={10 + (progress / 100) * 80} cy="24" r="3" fill="#f59e0b" />
        <text x={10 + (progress / 100) * 80} y="40" fontSize="7" fill="rgba(255,255,255,.8)" textAnchor="middle" fontWeight="bold">
          {year > 0 ? year : `${Math.abs(year)} BCE`}
        </text>
      </svg>
      <label className="text-xs text-white/70 block mt-2">Year
        <input type="range" min={-3000} max={2000} value={year} onChange={(e) => setYear(+e.target.value)} className="w-full" />
      </label>
      <div className="mt-2 text-xs text-white/60">Current period: {periodData.label} ({periodData.start > 0 ? periodData.start : `${Math.abs(periodData.start)} BCE`} - {periodData.end > 0 ? periodData.end : `${Math.abs(periodData.end)} BCE`})</div>
    </div>
  );
}

function LiteraryAnalysisSim() {
  const [complexity, setComplexity] = useState(60);
  const [themes, setThemes] = useState(4);
  
  // Literary analysis depth = complexity × themes (scaled)
  const analysisDepth = Math.min(100, (complexity / 100) * (themes / 5) * 100);
  const interpretation = Math.min(100, analysisDepth * 0.9);
  const criticalThinking = Math.min(100, analysisDepth * 1.1);
  
  return (
    <div className="rounded-2xl bg-white/5 ring-1 ring-white/10 p-4">
      <div className="mb-2 flex items-center gap-2 text-sm text-white/80">
        <PenTool className="h-4 w-4 text-amber-300" /> Literary Analysis
      </div>
      <div className="text-xs text-white/60 mb-3">Complexity: {complexity}%, Themes: {themes} → Depth: {analysisDepth.toFixed(0)}%</div>
      <svg viewBox="0 0 100 50" className="w-full h-32">
        {/* Analysis depth bar */}
        <rect x="10" y="8" width="80" height="8" fill="rgba(255,255,255,.1)" rx="2" />
        <rect x="10" y="8" width={(analysisDepth / 100) * 80} height="8" fill="#d97706" rx="2" />
        <text x="50" y="15" fontSize="7" fill="rgba(255,255,255,.8)" textAnchor="middle" fontWeight="bold">Analysis Depth: {analysisDepth.toFixed(0)}%</text>
        {/* Interpretation bar */}
        <rect x="10" y="20" width="80" height="8" fill="rgba(255,255,255,.1)" rx="2" />
        <rect x="10" y="20" width={(interpretation / 100) * 80} height="8" fill="#f59e0b" rx="2" />
        <text x="50" y="27" fontSize="7" fill="rgba(255,255,255,.8)" textAnchor="middle">Interpretation: {interpretation.toFixed(0)}%</text>
        {/* Critical thinking bar */}
        <rect x="10" y="32" width="80" height="8" fill="rgba(255,255,255,.1)" rx="2" />
        <rect x="10" y="32" width={(criticalThinking / 100) * 80} height="8" fill="#fbbf24" rx="2" />
        <text x="50" y="39" fontSize="7" fill="rgba(255,255,255,.8)" textAnchor="middle">Critical Thinking: {criticalThinking.toFixed(0)}%</text>
      </svg>
      <div className="mt-3 grid grid-cols-2 gap-2">
        <label className="text-xs text-white/70">Text Complexity (%)
          <input type="range" min={0} max={100} value={complexity} onChange={(e) => setComplexity(+e.target.value)} className="w-full" />
        </label>
        <label className="text-xs text-white/70">Themes Count
          <input type="range" min={1} max={10} value={themes} onChange={(e) => setThemes(+e.target.value)} className="w-full" />
        </label>
      </div>
      <div className="mt-2 text-xs text-white/60">Higher complexity and more themes deepen analysis</div>
    </div>
  );
}
...
function ResourceCard({ r, bookmarked, onBookmark }: { r: Resource; bookmarked?: boolean; onBookmark?: () => void }) {
  return (
    <motion.a href={r.link} target="_blank" rel="noreferrer" whileHover={{ y: -4 }}
      ...
      <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-amber-500/10 to-orange-500/10 opacity-0 transition-opacity group-hover:opacity-100" />
...
// --- Main History & Literature Hub ---
export default function HistoryLiteratureLearningHub() {
  const { discipline = "history-literature" } = useParams();
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        console.error("Error loading history & literature hub:", error);
        setLoading(false);
      }
    }
  ...
  useEffect(() => {
    ...
    const q = activeStage.quiz || [
      { q: `In "${activeStage.title}", what matters most?`, opts: ["Reading, writing & analytical skills", "Memorization", "Fashion", "Speed typing"], correct: "Reading, writing & analytical skills" },
      { q: "A key hist/lit skill is…", opts: ["Critical analysis & historical context", "Mime", "Crosswords", "Origami"], correct: "Critical analysis & historical context" },
    ];
      ...
      <div className="relative min-h-screen">
        ...
        <div className="grid place-items-center min-h-screen">
          <div className="flex items-center gap-3 text-white/80">
            <Loader2 className="h-5 w-5 animate-spin" /> Loading your History & Literature journey…
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="relative min-h-screen text-white">
      ...
      <header className="relative mx-auto max-w-6xl px-6 pt-14 text-center">
        <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={springy}>
          <div className="flex items-center justify-center gap-3 rounded-full bg-white/5 px-6 py-3 ring-1 ring-white/10 backdrop-blur mx-auto w-fit mb-6">
            ...
            <p className="font-semibold tracking-wide">History & Literature Learning Hub</p>
          </div>
          <h1 className="text-balance bg-gradient-to-br from-white to-amber-200 bg-clip-text text-4xl sm:text-6xl font-extrabold text-transparent">
            Explore the past, analyze texts, understand humanity
          </h1>
          <p className="max-w-2xl mx-auto mt-3 text-white/80">
            A comprehensive history and literature journey for future scholars, writers, and historians. Master analysis and critical thinking — right here.
          </p>
          ...
          <div className="mt-6 relative grid place-items-center mx-auto w-fit">
            <svg width="120" height="120" viewBox="0 0 120 120" className="drop-shadow">
              ...
              <motion.circle
                ...
                stroke="url(#histgrad)" strokeLinecap="round"
          ...
          {hub.stages.map((stage: any, idx: number) => {
            ...
            return (
              <li key={stage.id} className="ml-6 pb-10">
                <span className="absolute -left-[9px] mt-1 grid h-4 w-4 place-items-center rounded-full bg-amber-400 ring-2 ring-amber-300/50" />
                <motion.div whileHover={{ y: -2 }} className="rounded-2xl bg-white/5 p-5 backdrop-blur ring-1 ring-white/10">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => setActiveStage(stage)} className="inline-flex items-center gap-1 rounded-full bg-amber-500/20 px-3 py-1 text-amber-200 ring-1 ring-amber-300/30 hover:bg-amber-500/30">
              ...
              <div className="grid gap-6 p-6 sm:grid-cols-5">
                ...
                <div className="sm:col-span-3 space-y-6">
                  ...
                  <div className="rounded-2xl bg-black/30 p-4 ring-1 ring-white/10 backdrop-blur-md">
                    <h4 className="mb-3 font-semibold text-amber-200">Checklist</h4>
                  ...
                  <div className="rounded-2xl bg-black/30 p-4 ring-1 ring-white/10 backdrop-blur-md">
                    <h4 className="mb-3 font-semibold text-amber-200">Curated resources</h4>
                  ...
                  <div className="rounded-2xl bg-black/30 p-4 ring-1 ring-white/10 backdrop-blur-md">
                    <h4 className="mb-3 font-semibold text-amber-200">Reflection</h4>
                    <textarea
                      ...
                      placeholder="What historical events, literary works, or analytical insights did you explore?"
                ...
                <div className="sm:col-span-2 flex flex-col gap-4">
                  <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                    <div className="mb-2 flex items-center gap-2 text-sm text-white/80"><Sparkles className="h-4 w-4 text-amber-300" /> Scholarly Playground</div>
                  ...
                  <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                    <div className="mb-2 flex items-center gap-2 text-sm text-white/80"><GraduationCap className="h-4 w-4 text-amber-300" /> Quick quiz</div>
                    <div className="space-y-3">
                      {(activeStage.quiz || [
                        { q: `In "${activeStage.title}" what matters most?`, opts: ["Reading, writing & analytical skills", "Memorization", "Graphic design", "Speed typing"], correct: "Reading, writing & analytical skills" },
                        { q: "A key hist/lit skill is…", opts: ["Critical analysis & historical context", "Mime", "Crosswords", "Origami"], correct: "Critical analysis & historical context" },
                      ]).map((Q: any, i: number) => (
                        <div key={i}>
                            ...
                            {Q.opts.map((opt: string) => (
                              <button key={opt} onClick={() => setQuizAnswers((prev) => ({ ...prev, [i]: opt }))}
                                className={cx("rounded-full border border-white/15 px-3 py-1 text-sm",
                                  quizAnswers[i] === opt ? "bg-amber-500/30" : "bg-white/5 hover:bg-white/10")}>{opt}</button>
                            ))}
                          </div>
                        </div>
                      ))}
                  ...
                  {(activeStage.regions?.Global?.projectIdeas?.length > 0) && (
                    <div className="rounded-2xl bg-white/5 p-4 ring-1 ring-white/10">
                      <div className="mb-2 flex items-center gap-2 text-sm text-white/80"><BookOpen className="h-4 w-4 text-orange-300" /> Project ideas</div>
                      <div className="flex snap-x snap-mandatory overflow-x-auto gap-3 pb-2">
                        {activeStage.regions.Global.projectIdeas.map((idea: string, i: number) => (
                          <motion.div key={i} whileHover={{ y: -2 }} className="snap-start min-w-[280px] rounded-2xl bg-white/5 p-4 ring-1 ring-white/10 backdrop-blur">
                            <div className="mb-2 flex items-center gap-2 text-xs text-white/70"><Sparkles className="h-4 w-4 text-amber-200" /> Idea</div>
      ...
      <footer className="py-10 text-center text-xs text-white/50">Made for future historians & literary scholars 📜</footer>
    </div>
  );
}
